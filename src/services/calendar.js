...
export function exportCalendar(tableId) {
  const state     = getCampaignDate(tableId);
  const categories = db.prepare('SELECT * FROM calendar_categories WHERE table_id = ? ORDER BY id').all(tableId);
  const events    = db.prepare('SELECT * FROM calendar_events WHERE table_id = ? ORDER BY galactic_year, date_start').all(tableId);
  return {
...
export function importCalendar(tableId, userId, data, mode = 'merge') {
  const { categories = [], events = [], state } = data;

  const tx = db.transaction(() => {
    if (mode === 'replace') {
      db.prepare('DELETE FROM calendar_events WHERE table_id = ?').run(tableId);
      db.prepare('DELETE FROM calendar_categories WHERE table_id = ? AND is_system = 0').run(tableId);
    }

    // Restore campaign date if provided
    if (state?.date) {
      setCampaignDate(tableId, state.date, state.year ?? 50429);
    }

    // Build category id mapping: old id → new id
    const catMap = {};
    for (const cat of categories) {
      // Skip system categories — find by name
      const existing = db.prepare('SELECT id FROM calendar_categories WHERE table_id = ? AND name = ?').get(tableId, cat.name);
      if (existing) {
        catMap[cat.id] = existing.id;
      } else {
        const ins = db.prepare(
          'INSERT INTO calendar_categories (table_id, name, color, is_system) VALUES (?, ?, ?, ?) RETURNING id'
        ).get(tableId, cat.name, cat.color || '#6366f1', cat.is_system ? 1 : 0);
        catMap[cat.id] = ins.id;
      }
    }

    let inserted = 0, skipped = 0;
    for (const ev of events) {
      const exists = db.prepare('SELECT id FROM calendar_events WHERE id = ?').get(ev.id);
      if (exists) { skipped++; continue; }
      db.prepare(`
        INSERT INTO calendar_events
          (id, table_id, title, description, category_id, color, date_start, date_end, galactic_year, is_public, created_by)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `).run(
        ev.id, tableId, ev.title, ev.description || null,
        ev.category_id != null ? (catMap[ev.category_id] ?? null) : null,
        ev.color || null, ev.date_start || null, ev.date_end || null,
        ev.galactic_year ?? 50429, ev.is_public ? 1 : 0, userId
      );
      inserted++;
    }
    return { inserted, skipped, categories_mapped: Object.keys(catMap).length };
  });
  return tx();
}
